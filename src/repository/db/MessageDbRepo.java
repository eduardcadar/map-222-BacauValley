...
public class MessageDbRepo {
    ...
    public void save(Message message) {
        ...
        String sql = "INSERT INTO " + messagesTable + " (sender, messageText) VALUES (?, ?)";
        try (Connection connection = DriverManager.getConnection(url, username, password);
            PreparedStatement ps = connection.prepareStatement(sql)) {
    ...
    public void save(ReplyMessage message) {
        validator.validate(message);
        String sql = "INSERT INTO " + messagesTable + " (sender, messageText, idMsgRepliedTo) VALUES (?, ?, ?)";
        try (Connection connection = DriverManager.getConnection(url, username, password);
             PreparedStatement ps = connection.prepareStatement(sql)) {
            ps.setString(1, message.getSender());
            ps.setString(2, message.getMessage());
            ps.setInt(3, message.getIdMsgRepliedTo());
            ps.executeUpdate();
        } catch (SQLException throwables) {
            throw new DbException(throwables.getMessage());
        }
